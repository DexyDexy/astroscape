...
(function (NS) {
  ...
  const ru = {
    ...
    ui: {
      ...
      gps: '◎ Определить по GPS', bootFail: 'Не удалось запустить 3D‑сцену. Обновите браузер (на iPhone — iOS 16.4 или новее) и проверьте, что не блокируется загрузка скриптов.', apply: 'Применить', pickOnMap: '◉ Выбрать на карте', cancel: 'Отмена',
      ...
      privacyNote: 'Ничего не отправляется в сеть: место хранится только в этом браузере. GPS запрашивается только по вашему нажатию, координаты округляются примерно до километра.',
  ...
  const en = {
    ...
    ui: {
      ...
      gps: '◎ Use GPS', bootFail: 'The 3D scene could not start. Update your browser (on iPhone, iOS 16.4 or newer) and check that script loading is not blocked.', apply: 'Apply', pickOnMap: '◉ Pick on the map', cancel: 'Cancel',
      ...
      privacyNote: 'Nothing is sent anywhere: the location is stored only in this browser. GPS is requested only when you press the button, and coordinates are rounded to about a kilometre.',
